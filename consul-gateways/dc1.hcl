...
container "consul_gateway_dc1" {
  ...
  env {
    ...
    value = "consul_dc1.container.shipyard:8500"
  }
  
  env {
    key = "CONSUL_GRPC_ADDR"
    value = "consul_dc1.container.shipyard:8502"
  }
}

# Ingress to allow local access to the Consul UI for the DC1 server
ingress "consul_http_dc1" {
...
container "web_dc1" {
  ...
  env {
    key = "CONSUL_SERVER"
    value = "consul_dc1.container.shipyard"
  }
